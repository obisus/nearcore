...
use near_vm_logic::{ProtocolVersion, ReturnData, VMConfig, VMContext, VMKind, VMOutcome};
...
impl Script {
    ...
    pub(crate) fn profile(&mut self, yes: bool) {
        self.profile = if yes { Some(ProfileData::new()) } else { None }
    }
